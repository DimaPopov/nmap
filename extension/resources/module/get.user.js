...
(function () {
  ...
  const renderUsersList = (users, isUpdate = false) => {
    ...
    users.forEach((user) => {
      ...
      if (!!icon.length) {
        ...
        iconRole.hover(() => {
          popup.find(".nk-popup__content").text(textIcon);

          const topPopup = iconRole[0].offsetHeight + iconRole.offset().top + 5;
          const leftPopup = window.innerWidth - iconRole.offset().left;
  ...
  */
  ...
  const showViewGetUser = (isEdit = true) => {    
    ...
    /* Заготовка под всплывающее окно */
    $(".nk-portal-local").remove();

    $("body").append('<div class="nk-portal nk-portal-local"><!----><div class="nk-popup nk-popup_direction_bottom-left nk-popup_theme_islands nk-popup_view_tooltip" style="z-index: 111001;"><div class="nk-size-observer"><div class="nk-popup__content"></div></div></div><!----><!----></div>');
    const popup = $(".nk-portal-local .nk-popup");

...
(function () {
  ...
  const getUserAddress = (textarea) => {
    ...
    return value
      ...
      .replaceAll(/владение (\d)|влад (\d)|вл (\d)/g, "вл$1$2$3")
      /* Укажем литеры в соответствии с правилами */
      .replaceAll(/(\d) ([А-ЯЁ])/g, "$1$2")
      /* Укажем строения в соответствии с правилами */
      .replaceAll(/\sстроение (\d)|\sстр (\d)|\sс (\d)/g, "с$1$2$3")
      /* Укажем корпуса в соответствии с правилами */
      .replaceAll(/\sкорпус (\d)|\sкорпус ([А-ЯЁ])|\sкор (\d)|\sкор ([А-ЯЁ])|\sк (\d)|\sк ([А-ЯЁ])/g, "к$1$2$3$4$5$6")
